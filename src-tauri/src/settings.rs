...
/// Maximum number of post-process prompts a user can keep.
pub const MAX_POST_PROCESS_PROMPTS: usize = 5;

/// Binding-id prefix for per-prompt post-process hotkeys (`post_process:{prompt_id}`).
pub const POST_PROCESS_BINDING_PREFIX: &str = "post_process:";
...
/// Build the internal shortcut id for a post-process prompt.
pub fn post_process_binding_id(prompt_id: &str) -> String {
    format!("{}{}", POST_PROCESS_BINDING_PREFIX, prompt_id)
}

/// Extract the prompt id from a `post_process:{id}` binding id.
pub fn parse_post_process_prompt_id(binding_id: &str) -> Option<&str> {
    binding_id.strip_prefix(POST_PROCESS_BINDING_PREFIX)
}

/// Platform default hotkey for a fresh post-process prompt.
...
/// Synthesize a [`ShortcutBinding`] for a prompt that has a hotkey assigned.
pub fn shortcut_binding_for_prompt(prompt: &LLMPrompt) -> Option<ShortcutBinding> {
    if prompt.binding.trim().is_empty() {
        return None;
    }
    Some(ShortcutBinding {
        id: post_process_binding_id(&prompt.id),
        name: prompt.name.clone(),
        description: format!("Transcribe and post-process with '{}'", prompt.name),
        default_binding: prompt.binding.clone(),
        current_binding: prompt.binding.clone(),
    })
}
...
fn default_post_process_prompts() -> Vec<LLMPrompt> {
    vec![LLMPrompt {
        ...
        binding: default_post_process_shortcut().to_string(),
    }]
}
...
/// Migrate the legacy single post-process hotkey onto individual prompts.
///
/// Copies `bindings.transcribe_with_post_process` onto the previously selected
/// prompt (or the first prompt), then removes the global binding entry.
fn migrate_per_prompt_hotkeys(settings: &mut AppSettings) -> bool {
    let mut changed = false;

    let legacy_binding = settings
        .bindings
        .get("transcribe_with_post_process")
        .map(|b| b.current_binding.clone());

    if let Some(hotkey) = legacy_binding {
        let target_id = settings
            .post_process_selected_prompt_id
            .clone()
            .or_else(|| settings.post_process_prompts.first().map(|p| p.id.clone()));
...
fn ensure_post_process_defaults(settings: &mut AppSettings) -> bool {
    let mut changed = migrate_per_prompt_hotkeys(settings);
    for provider in default_post_process_providers() {
...
mod tests {
    ...
    #[test]
    fn legacy_prompt_without_binding_deserializes_empty() {
        let prompt: LLMPrompt = serde_json::from_value(serde_json::json!({
            "id": "default",
            "name": "Default",
            "prompt": "Clean up"
        }))
        .expect("binding should default");
        assert_eq!(prompt.binding, "");
    }

    #[test]
    fn post_process_binding_id_round_trips() {
        assert_eq!(
            post_process_binding_id("prompt_1"),
            "post_process:prompt_1"
        );
        assert_eq!(
            parse_post_process_prompt_id("post_process:prompt_1"),
            Some("prompt_1")
        );
        assert_eq!(parse_post_process_prompt_id("transcribe"), None);
    }

    #[test]
    fn migrate_moves_global_hotkey_to_selected_prompt() {
        let mut settings = get_default_settings();
        ...
        settings.post_process_selected_prompt_id = Some("b".into());
        ...
        assert!(migrate_per_prompt_hotkeys(&mut settings));
    ...
    #[test]
    fn migrate_is_idempotent_after_first_run() {
        let mut settings = get_default_settings();
        settings.bindings.insert(
            "transcribe_with_post_process".into(),
            ShortcutBinding {
    ...
    #[test]
    fn default_prompt_includes_platform_hotkey() {
        let prompts = default_post_process_prompts();
        assert_eq!(prompts.len(), 1);
        assert!(!prompts[0].binding.is_empty());
        assert_eq!(prompts[0].binding, default_post_process_shortcut());
    }

    #[test]
    fn default_bindings_exclude_legacy_post_process_hotkey() {
        let defaults = get_default_settings();
        assert!(!defaults
            .bindings
